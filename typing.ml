...
exception Error of Type.t * Type.t * int * int
...
let rec deref_term = function
  ...
  | ExprWithPos(e, p) -> ExprWithPos(deref_term e, p)
  | LetGlobal(xt, e1, e2) -> LetGlobal(deref_id_typ xt, deref_term e1, deref_term e2)
  | e -> e
...
let rec rmpos e = 
    ...
    | Not(e) -> Not(rmpos e)
    | Neg(e) -> Neg(rmpos e)
    | Add(e1, e2) -> Add(rmpos e1, rmpos e2)
    | Sub(e1, e2) -> Sub(rmpos e1, rmpos e2)
    | Mul(e1, e2) -> Mul(rmpos e1, rmpos e2)
    | Div(e1, e2) -> Div(rmpos e1, rmpos e2)
    | Eq(e1, e2) -> Eq(rmpos e1, rmpos e2)
    | NEq(e1, e2) -> NEq(rmpos e1, rmpos e2)
    | LE(e1, e2) -> LE(rmpos e1, rmpos e2)
    | LT(e1, e2) -> LT(rmpos e1, rmpos e2)
    | FNeg(e) -> FNeg(rmpos e)
    | FAdd(e1, e2) -> FAdd(rmpos e1, rmpos e2)
    | FSub(e1, e2) -> FSub(rmpos e1, rmpos e2)
    | FMul(e1, e2) -> FMul(rmpos e1, rmpos e2)
    | FDiv(e1, e2) -> FDiv(rmpos e1, rmpos e2)
    | If(e1, e2, e3) -> If(rmpos e1, rmpos e2, rmpos e3)
    | Let(xt, e1, e2) -> Let(xt, rmpos e1, rmpos e2)
    | LetGlobal(xt, e1, e2) -> LetGlobal(xt, rmpos e1, rmpos e2)
    | LetRec({ name = xt; args = yts; body = e1 }, e2) ->
        LetRec({ name = xt;
                args = yts;
                body = rmpos e1 },
                rmpos e2)
    | App(e, es) -> App(rmpos e, List.map rmpos es)
    | Tuple(es) -> Tuple(List.map rmpos es)
    | LetTuple(xts, e1, e2) -> LetTuple(xts, rmpos e1, rmpos e2)
    | Array(e1, e2) -> Array(rmpos e1, rmpos e2)
    | Get(e1, e2) -> Get(rmpos e1, rmpos e2)
    | Put(e1, e2, e3) -> Put(rmpos e1, rmpos e2, rmpos e3)
    | ExprWithPos(e, p) -> rmpos e
    | e -> e
...
let rec g env e = (* 型推論ルーチン (caml2html: typing_g) *)
    ...
    | ExprWithPos (e, p) ->
        (try 
             let t = g env e in t
         with 
             Unify(t1, t2) -> failwith 
                              (Printf.sprintf "type error near line %d, offset %d, %s and %s is different type" 
                               (fst p)
                               (snd p)
                               (Type.type2string (deref_typ t1))
                               (Type.type2string (deref_typ t2)) ))
    | Fun(_, _) -> failwith("Fun never reachable here")
