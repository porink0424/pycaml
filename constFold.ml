...
let rec pow value idx =
  if idx <= 0 then 1
  else value * pow value (idx - 1)
...
let rec g env = function (* 定数畳み込みルーチン本体 (caml2html: constfold_g) *)
  ...
  | Mul(x, y) when memi x env && memi y env -> Int(findi x env * (pow 2 (findi y env)))
  | Div(x, y) when memi x env && memi y env -> Int(findi x env / (pow 2 (findi y env)))
  ...
  | IfNEq(x, y, e1, e2) when memi x env && memi y env -> if findi x env <> findi y env then g env e1 else g env e2
  | IfNEq(x, y, e1, e2) when memf x env && memf y env -> if findf x env <> findf y env then g env e1 else g env e2
  | IfNEq(x, y, e1, e2) -> IfNEq(x, y, g env e1, g env e2)
  ...
  | ExprWithPos(e, pos) -> ExprWithPos(g env e, pos)
  | e -> e
