...
(* オブジェクトの個数 *)
let global n_objects = Array.make 1 0
in

(* オブジェクトのデータを入れるベクトル（最大60個）*)
let global dummy = Array.make 0 0.0 in
let global objects = 
  Array.make 60 (0, 0, 0, 0, dummy, dummy, false, dummy, dummy, dummy, dummy)
in

(* Screen の中心座標 *)
let global screen = Array.make 3 0.0
in
(* 視点の座標 *)
let global viewpoint = Array.make 3 0.0
in
(* 光源方向ベクトル (単位ベクトル) *)
let global light = Array.make 3 0.0
in
(* 鏡面ハイライト強度 (標準=255) *)
let global beam = Array.make 1 255.0
in
(* AND ネットワークを保持 *)
let global and_net = Array.make 50 (Array.make 1 (-1))
in
(* OR ネットワークを保持 *)
let global or_net = Array.make 1 (Array.make 1 (and_net.(0)))
in

(* 以下、交差判定ルーチンの返り値格納用 *)
(* solver の交点 の t の値 *)
let global solver_dist = Array.make 1 0.0
in
(* 交点の直方体表面での方向 *)
let global intsec_rectside = Array.make 1 0
in
(* 発見した交点の最小の t *)
let global tmin = Array.make 1 (1000000000.0)
in
(* 交点の座標 *)
let global intersection_point = Array.make 3 0.0
in
(* 衝突したオブジェクト番号 *)
let global intersected_object_id = Array.make 1 0
in
(* 法線ベクトル *)
let global nvector = Array.make 3 1.0
in
(* 交点の色 *)
let global texture_color = Array.make 3 0.0
in

(* 計算中の間接受光強度を保持 *)
let global diffuse_ray = Array.make 3 0.0
in
(* スクリーン上の点の明るさ *)
let global rgb = Array.make 3 0.0
in

(* 画像サイズ *)
let global image_size = Array.make 2 0
in
(* 画像の中心 = 画像サイズの半分 *)
let global image_center = Array.make 2 0
in
(* 3次元上のピクセル間隔 *)
let global scan_pitch = Array.make 1 0.0
in

(* judge_intersectionに与える光線始点 *)
let global startp = Array.make 3 0.0
in
(* judge_intersection_fastに与える光線始点 *)
let global startp_fast = Array.make 3 0.0
in

(* 画面上のx,y,z軸の3次元空間上の方向 *)
let global screenx_dir = Array.make 3 0.0
in
let global screeny_dir = Array.make 3 0.0
in
let global screenz_dir = Array.make 3 0.0
in

(* 直接光追跡で使う光方向ベクトル *)
let global ptrace_dirvec  = Array.make 3 0.0
in

(* 間接光サンプリングに使う方向ベクトル *)
let global dummyf = Array.make 0 0.0 in
let global dummyff = Array.make 0 dummyf in
let global dummy_vs = Array.make 0 (dummyf, dummyff) in
let global dirvecs = 
  Array.make 5 dummy_vs
in

(* 光源光の前処理済み方向ベクトル *)
let global dummyf2 = Array.make 0 0.0 in
let global v3 = Array.make 3 0.0 in
let global consts = Array.make 60 dummyf2 in
let light_dirvec =
  (v3, consts)
in

(* 鏡平面の反射情報 *)
let global dummyf3 = Array.make 0 0.0 in
let global dummyff3 = Array.make 0 dummyf3 in
let dummydv = (dummyf3, dummyff3) in
let global reflections =
  Array.make 180 (0, dummydv, 0.0)
in

(* reflectionsの有効な要素数 *) 

let global n_reflections = Array.make 1 0
in
